...
@section('title') Edit Email Marketing Template - CRM @endsection
@section('content')
    <style>
        .required{
            color:red;
                ...
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <div class="page-header" style="border:none">
                            <h3 class="page-title">Edit Email Marketing Template</h3>
                        </div>
                    ...
                    <div class="col-sm-12">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class=""><a  href="{{ route('admin.email.template') }}" style="color: #fff; margin-right:20px" class="btn btn-info btn-sm"><i class="fa fa-list"></i> Email Template List</a></li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
            ...
            <div class="card">
                ...
                <div class="card-body p-0" style="margin-top: 30px;margin-bottom: 30px;">
                    <table class="table table-striped projects">
                        <form method="POST" action="{{ route('admin.email.template.update') }}" enctype="multipart/form-data">
                            @csrf

                              <div id="nt_area">
                                <div class="form-group row">
                                    <label for="" class="col-md-2 col-form-label text-md-right">{{ __('Email Template Type') }}</label>
                                    <div class="col-md-6">
                                        <select name="nt_id" class="form-control" required>
                                            ...
                                            @foreach($notificationsTypes as $notificationsType)
                                                @if($notificationsType->name != null)
                                                    <option value="{{$notificationsType->id}}" @if ($notificationsType->id== $notificationTemplate->temp_type_id){{'selected'}}  @endif>{{$notificationsType->name}}</option>
                                                @else
                            ...
                            <div class="form-group row">
                                <label for="division_id" class="col-md-2 col-form-label text-md-right">{{ __('Email Template') }}</label>
                                <div class="col-md-6">
