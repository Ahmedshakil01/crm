      ...
      <div class="row mb-2">
        <div class="col-sm-6">
          <div class="page-header" style="border:none">
            <h3 class="page-title">View Note List</h3>
        </div>
        ...
        <div class="col-sm-12">
          ...
          @if($permission->can('news.create'))
            ...
            <i class="fa fa-plus"></i> Add note</a></li>
          @endif
          </ol>
        </div>
        @endif
          ...
          <div class="modal-dialog" role="document">
            ...
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Note for {{ $type}}</h5>
              ...
              <div class="modal-body">
                  <div class="col-sm-12"> 
                        	<div class="form-group row"><label for="name">Subject</label><input type="text" name="caption" class="form-control" placeholder="Caption" /></div>  
            ...
            <table id="responsive-datatable" class="table table-striped table-bordered" cellspacing="0" width="100%">
              ...
              <tr id="tablerow<?php echo $lead->id;?>" class="tablerow">
               ...
               @endif
                ...
                <td>
                 @if($lead->files!="")
                      <a href="{{ route('attachment.samplefiledownload',['file_names'=>$lead->files,'type'=>$type,'path'=>'uploads/note']) }}" 
                ...
                <td align="right">   
                  @if($permission->can('news.edit'))
                    <a  href="#" class="btn btn-primary btn-xs" onclick="noteModal('{{ $lead->id }}','{{ $lead->caption }}','{{ $lead->details }}','{{ $lead->files }}')"><i class="fa fa-edit"></i></a>
                  @endif
                                
                  @if($permission->can('news.delete'))
                    <button type="button" class="btn btn-xs btn-danger" style="margin-top:5px;"
                      onclick="deleteSingle('<?php echo $lead->id;?>','masterdelete','notes')"><i class="fa fa-trash"></i></button>
                  @endif
                </td>
                @endif
              </tr>
...
<div class="modal fade" id="noteEditModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
                  ...
                  <div class="col-sm-12"> 
                            ...
                            <div class="form-group row">
                            	 <label for="name">Attachment</label>
                                  ...
                                  <input type="hidden" class="form-control" name="noteid" id="noteid">
                                  <input type="hidden" name="type" value="{{ $type }}" />
                                  <input type="hidden" name="id" value="{{ $id }}" />
...
@endsection
<script>
	function noteModal(id,subject,details,stillimg){
		$("#noteEditModal").modal();
		$("#noteid").val(id);
		$("#subject").val(subject);
		$("#details").val(details);
